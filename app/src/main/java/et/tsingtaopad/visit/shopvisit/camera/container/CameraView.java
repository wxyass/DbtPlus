...
public class CameraView extends SurfaceView implements CameraOperation{
	...
	protected void onFocus(Point point,AutoFocusCallback callback){
		...
		try {
			//不同手机 在设置聚焦区域的时候经常会出异常，看日志发现是框架层的字符串转int的时候出错了，
			//目测是 修改了框架层代码导致，在此try掉，对实际聚焦效果没影响
			mCamera.setParameters(parameters);
		} catch (Exception e) {
	...
	private void setCameraParameters(){
		...
		// 保存时大小
		parameters.setPictureSize(640,480);//width， height
	...
	private void updateCameraOrientation(){
		if(mCamera!=null){
			...
			//预览图片旋转90°
			if(mIsFrontCamera){// 前置 旋转
				if(Build.MODEL.contains("A3000")){
					mCamera.setDisplayOrientation(270);//预览转90°
				}else{
