...
public class MakePlanActivity extends BaseActivity implements OnClickListener, OnItemSelectedListener
{
    ...
    private String amStartStr = "08:00", amEndStr = "12:00",
            pmStartStr = "13:00", pmEndStr = "18:00";
 ...
 // 供货关系弹出框相关实例
    ...
    {
        super.onCreate(arg0);
        setContentView(R.layout.operation_makeplan);
        //制定工作计划业务处理
        ...
        Thread thread = new Thread()
        {
            ...
            public void run()
            {
                ...
                try
                {
                    ...
                    //					queryBuilder.where().ge("enddate", planDate.replace("-", "")).and().le("startdate", planDate.replace("-", ""));
                    //					validPromotions = queryBuilder.query();
                    
                    
                    // 查询空白终端添加产品list
                    //List<MstProductM> products = service.queryProduct(MakePlanActivity.this, ConstValues.loginSession.getGridId(), ConstValues.loginSession.getDisId());
                    List<MstProductM> products = service.queryProduct(MakePlanActivity.this, PrefUtils.getString(getApplicationContext(), "gridId", ""), PrefUtils.getString(getApplicationContext(), "disId", ""));
                ...
                {
                    handler.sendEmptyMessage(INITFINISH);//1 onCreate:获取上一页面传递过来的数据
                    									 //2 initDate:查数据库获取本页面所需基础数据
                    									 //3 发送handler触发initView();  initViewData(); 
                }
            }
        };
        thread.start();
    ...
    private void initViewPager()
    {
        ...
        //指标list
        if (planTempChecks != null && !planTempChecks.isEmpty())// 计划指标集合planTempChecks 是查PlantempcheckM表得到的
        {
            //如果是预览的,隐藏确定按钮
            if (preview_plan)
            {
            ...
            for (int i = 0; i < planTempChecks.size(); i++)
            {
                ...
                // 产品生动化,冰冻化
                    ...
                    tv_name.setText(getString(R.string.makeplan_productname));//并把   名称  改为   采集项名称
                    tv_produname.setText(getString(R.string.makeplan_proname));//并把   名称  改为   产品名称
                    if (preview_plan)
                    {
                ...
                // 竞品清除计划(QC竞品)
                	...
                	tv_produname.setVisibility(View.VISIBLE);
                    btn_add.setTag(checkname);
                    tv_name.setText(getString(R.string.makeplan_pinpai));//并把   名称  改为    品牌
                    tv_produname.setText(getString(R.string.makeplan_proname));//并把   名称  改为 产品名称 
                    if (preview_plan)
                    {
                        btn_add.setVisibility(View.GONE);
                ...
                {
                    tv_name.setText(getString(R.string.makeplan_name));//不是空白终端时     tv_name赋值为  名称      添加按钮也看不见
                    btn_add.setVisibility(View.GONE);
                }
                ...
                //显示采集项下面的item
                MakePlanViewPagerAdapter adapter = new MakePlanViewPagerAdapter(this, vpLvItemStcS, checkKey, lineKeys, 
                		preview_plan, isProductTerminal, isPromotionhua, isotherhua, isGezhonghua, isjingpintuijin, isDaojuhua,isChanpinhua,isBingdonghua,isdingdan);
    ...
    private void initDataViewpager()
    {
        ...
        else
        {
            ...
            try
            {
            	...
            	// 解释: 每确定制定一天的日计划会在MstPlancheckInfo表中生成多条记录(有几个计划指标,就生成几条记录,以plankey做关联),
                ...
                if (!planTempChecks.isEmpty())
                {
                    for (int i = 0; i < planTempChecks.size(); i++)
                    {
                        ...
                        for (int j = 0; j < plancheckInfos.size(); j++)
                        {// 不在查数据库
                         // 通过所有数据循环对比
                        ...
                        for (int j = 0; j < plancollectionInfos.size(); j++)
                        {
                            ...
                            if(!TextUtils.isEmpty(remarks)){// 如果remarks为空,表示该采集项没有选择终端 就不处理 // ywm 2016/04/05
                            	
                            	selectTerminalsMap.put(pcolitemkey, JsonUtil.parseList(remarks, String.class));//不太明白
                            }

                            // 0采集项1产品2活动3有效铺货4有效销售5竞品推进计划品牌
                            //if (planType_blank.equals(plantype) || planType_puhuo.equals(plantype) || planType_xiaoshou.equals(plantype))
                            	...
                            	|| dingDanCheckkey.equals(checkkey))
                            {
                                ...
                                if (!preview_plan)// 预览状态下就不执行移除产品 ，因为不需要选择产品
                                {
                                    ...
                                    //}else if(planType_puhuo.equals(plantype)){//有效铺货
                            ...
                            else if ("884d0d53-2cdb-4214-b24d-137d8753047p".equals(checkkey))// 促销活动推进
                            {
                            ...
                            }else if("a16ccbc3-4be7-42b8-a338-56da74c6ddf4".equals(checkkey)){// 竞品清除计划(QC竞品) 未改
                            	...
                            	if((!CmproductinfoM.isEmpty())&&mstCmpcompanyM.size()>0){
                                	...
                                	VpLvItemStc item = new VpLvItemStc();
                                    item.setPcolitemkey(pcolitemkey);
                                    item.setName(mstCmpcompanyM.get(0).getCmpcomname());// 竞品清除计划(QC竞品) 品牌
                                    item.setKey(plancollectionInfo.getColitemkey());// 竞品清除计划(QC竞品) 品牌key
                                    item.setProname(CmproductinfoM.get(0).getCmpproname());// 竞品清除计划(QC竞品) 产品名称
                                    item.setPronamekey(plancollectionInfo.getProductkey());// 竞品清除计划(QC竞品) 产品名称key
                        ...
                        // 促销活动 没终端的 
                        // 解释: 因为日计划确定后,只会将每个计划指标下有终端的记录保存到MstPlancollectionInfo表中,没有终端的不保存
                        //		所以修改模式下 其他,促销活动,道具生动化3个计划指标从MstPlancollectionInfo读到的数据,可能不是全面的
...
//    	
    ...
    private void savePlan()
    {
        ...
        try
        {
            ...
            connection.setAutoCommit(false);//屏蔽掉自动提交模式只有调用 connection.commit();才会提交
            ...
            String plantempkey = planTempChecks.get(0).getPlantempkey();//对应计划模板主键 , 多个计划指标对应一个计划模板主键
            ...
            {// b
                ...
                if (plancheckInfoDatas != null && plancheckInfoDatas.size() > 0)
                {
                    ...
                    {// c 1  2
                     // plancheckInfoDatas// 0152e64bc6f447c0a18b0413ed486cfd
                        ...
                        {//1  2
                         //指定
                        	...
                        	// 其他 促销活动推进 为第一档
                        	/*
                        	if(getResources().getString(R.string.plan_promotion).equals(viewPagertitle)
                        			|| getResources().getString(R.string.plan_other).equals(viewPagertitle)){
                        		
                        		plancollectionInfo.setCredate(createDate);//
                                plancollectionInfo.setPcheckkey(pcheckkey);// 计划指标主键 // pad端生成的随机数
                                ...
                                plancollectionInfo.setProductkey(plancheckInfoDatas.get(j).getPronamekey());// 道具生动化 产品名称key
                                ...
                                else if (getResources().getString(R.string.plan_promotion).equals(viewPagertitle))
                                {// 促销活动推进
                                    plancollectionInfo.setPlantype("2");
                                }
                                else if (getResources().getString(R.string.plan_puhuotitle).equals(viewPagertitle))
                                {// 有效铺货目标终端
                                ...
                                else if (getResources().getString(R.string.plan_xiaoshoutitle).equals(viewPagertitle))
                                {// 有效销售目标终端
                                ...
                                else if (getResources().getString(R.string.plan_jingpintitle).equals(viewPagertitle))
                                {// 竞品推进计划
                                	plancollectionInfo.setPlantype("5");
                                }
                                else if (getResources().getString(R.string.plan_daojutitle).equals(viewPagertitle)
                                		||getResources().getString(R.string.plan_product).equals(viewPagertitle)
                                		||getResources().getString(R.string.plan_bingdongtitle).equals(viewPagertitle))
                                {// 各种化
                        ...
                        {// 修改模式
                        	...
                        	//if(isDaoJu(viewPagertitle) || isChanPin(viewPagertitle) || isBingDong(viewPagertitle)){
                        if( isChanPin(viewPagertitle) || isBingDong(viewPagertitle)){
                                ...
                                if (plancollectionInfos.isEmpty())
                                {
                                	if(Long.parseLong(plancheckInfoDatas.get(j).getNum())>0){// 终端数目大于0时才保存
                                        ...
                                        plancollectionInfo.setUpdateuser(userCode);
                                        plancollectionInfo.setCreuser(userCode);
                                        if (isBlank(viewPagertitle))
                                        {// 空白终端
                                         // 产品主键
                                            plancollectionInfo.setPlantype(planType_blank);
                                        }
                                        else if (isPuhuo(viewPagertitle))
                                        {// 有效铺货目标终端
                                            plancollectionInfo.setPlantype("3");
                                        }
                                        else if (isXiaoShou(viewPagertitle))
                                        {// 有效销售目标终端
                                ...
                                if (plancollectionInfos.isEmpty())
                                {
                                	if(Long.parseLong(plancheckInfoDatas.get(j).getNum())>0){// 终端数目大于0
                                        ...
                                        plancollectionInfo.setTermnum(Long.parseLong(plancheckInfoDatas.get(j).getNum()));// 数量
                                        plancollectionInfo.setTermnames(plancheckInfoDatas.get(j).getTerm());// 终端名称
                                        plancollectionInfo.setProductkey(plancheckInfoDatas.get(j).getPronamekey());// 竞品清除计划(QC竞品) 产品名称key
                                        plancollectionInfo.setColitemkey(colitemkey);// 空白终端产品主键/采集项主键
                                        plancollectionInfo.setUpdateuser(userCode);
                                        plancollectionInfo.setCreuser(userCode);
                                        if (isBlank(viewPagertitle))
                                        ...
                                        else if (isJingPin(viewPagertitle))
                                        {// 有效销售目标终端
                                        	plancollectionInfo.setPlantype("5");
                                        }
                                        else if (getResources().getString(R.string.plan_daojutitle).equals(viewPagertitle)
                        ...
                        // 对应的回显list的数据出错
                        List<String> list = selectTerminalsMap.get(pcolitemkey);// 该pcolitemkey采集项对应的 选择的终端key
                        ...
                        if (!CheckUtil.IsEmpty(list))
                        {
                            List<String> colitemkeys = new ArrayList<String>(list.size());//  1   2
                            for (int k = 0; k < list.size(); k++)
                            {
    ...
    /***
     * 保存计划指标信息
     * @param pcheckkey    计划主键
     * @param pcheckkey  计划指标主键
     * @param checkkey   指标主键
     * @param createDate 时间
     * @param userCode   用户编码
     * @param i
    ...
    public void ShowDiologWindow(final CopyOnWriteArrayList<MstProductM> productList, final String checkname) {
        ...
        // 加载弹出窗口layout
        itemForm = MakePlanActivity.this.getLayoutInflater()
                        .inflate(R.layout.operation_addrelation,null);
        productDialgo = new AlertDialog.Builder(
        				//设置对话框不能被取消
        		MakePlanActivity.this).setCancelable(false).create();
        //设置视图显示在对话框中
        ...
        for (int i = 0; i < planTempChecks.size(); i++)// 遍历每个指标
		{
			if (checkname.equals(planTempChecks.get(i).getCheckname())) {//
				...
				List<PadPlantempcollectionInfo> colitemList = service
						.queryColitemname(checkkey);// pad端计划模板指标对应采集项关联信息表的数据
				if (colitemList.size() > 0) {
					// 指标选项
					...
					caijixiangAdapter = new ListViewProductAdapter(this,
							vpLvItemStcS);
					...
					proLv.setOnItemClickListener(new OnItemClickListener() {
						...
						public void onItemClick(AdapterView<?> parent,
							...
							productes = new ArrayList<MstProductM>();
							pro = (VpLvItemStc) proLv
									.getItemAtPosition(position);
							if (!CheckUtil.IsEmpty(vpLvItemStcS)) {
								product1 = productList.get(0);
							}
							caijixiangproductAdapter = new MakePlanProductAdapter(
									MakePlanActivity.this, productList);
					...
					productLv1.setOnItemClickListener(new OnItemClickListener() {
						...
						public void onItemClick(AdapterView<?> parent,
								View view, int position, long id) {
							product1 = (MstProductM) productLv1
									.getItemAtPosition(position);
					...
					sureBt.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View arg0) {
							...
							if (productes.size() == 0) {
								...
								if (isDaoJu(checkname)) {// 添加道具 产品名称
									...
									for (MstProductM product : productes) {
										if (productnames.size() > 0&& productnames.contains(product.getProname())) {
											Toast.makeText(MakePlanActivity.this,getString(R.string.addrelation_msg_repetitionadd),
								...
								if (isJingPin(checkname)) {// 添加竞品清除计划(QC竞品) 产品名称
					...
					Button cancelBt = (Button) itemForm
							.findViewById(R.id.addrelation_bt_cancel);
    ...
    public void dialogProduct() {
        ...
        // 加载弹出窗口layout
        itemForm = MakePlanActivity.this.getLayoutInflater().inflate(R.layout.shopvisit_addrelation,null);
        productDialgo = new AlertDialog.Builder(MakePlanActivity.this).setCancelable(false).create();
        productDialgo.setView(itemForm, 0, 0, 0, 0);
        productDialgo.show();
        ...
        agencyAdapter = new ListViewKeyValueAdapter(MakePlanActivity.this,ConstValues.agencyVieLst, new String[]{"key","value"}, 
        ...
        agencyLv.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, 
                ...
                productAdapter = new ChatVieProductAdapter(
                		MakePlanActivity.this,agency.getChildLst(), new String[]{"key","value"}, 
        ...
        if (!CheckUtil.IsEmpty(ConstValues.agencyVieLst)) {
            productAdapter = new ChatVieProductAdapter(MakePlanActivity.this,
                	...
                	*/
						...
						if (jingpinVpLvItemStcs.size() > 0) {
							...
							for (VpLvItemStc vpLvItemStc : jingpinVpLvItemStcs) {
								// 用户选择的采集项 与原有条目中采集项是一样的 添加到集合中
								// 寻找原有的采集项
								/*if (agency.getName().equals(vpLvItemStc.getName())) {
									productnames.add(vpLvItemStc.getProname());// 添加产品名称
								}*/
