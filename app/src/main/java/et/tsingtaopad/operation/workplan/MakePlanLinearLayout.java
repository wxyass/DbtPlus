...
public class MakePlanLinearLayout extends BaseActivity implements OnClickListener, OnItemSelectedListener {
	...
	private ListView mListView;// 终端数量统计
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		setContentView(R.layout.operation_dayplan_linearlayout);
		// 制定工作计划业务处理
		...
		Thread thread = new Thread() {
			...
			public void run() {
				...
				try {
					...
					List<MstProductM> products = service.queryProduct(MakePlanLinearLayout.this, PrefUtils.getString(getApplicationContext(), "gridId", ""), PrefUtils.getString(getApplicationContext(), "disId", ""));
	...
	private void initView() {
		...
		// 底部计划模块容器
		makeplan_botton_ll = (LinearLayout) findViewById(R.id.makeplan_botton_ll);
	...
	/**
	 * 初始化底部LinearLayout,用不同计划指标模块填充
	 */
	private void initListView() {

		// viewPager中viewList (viewList:这是个list集合),有几个计划指标(废弃)
		viewList.clear();
		...
		if (planTempChecks != null && !planTempChecks.isEmpty())// 计划指标集合planTempChecks是查PlantempcheckM表得到的
		{
			...
			// 初始化每个计划指标模块的数据(每一个计划指标模块,是一个listview, 该方法为每个listview设置数据)
			initDataViewpager();
			
			// 上面是先把数据创建出来, 下面直接用
			
			
			makeplan_botton_ll.removeAllViews();
			
			// 填充底部容器
			for (int position = 0; position < planTempChecks.size(); position++) {
				
				View convertView = View.inflate(MakePlanLinearLayout.this, R.layout.operation_makeplan_listitem, null);
				
				// 计划指标
				TextView tv_planname = (TextView) convertView.findViewById(R.id.vp_tv_title_planname);
				// 采集项名称
				TextView tv_name = (TextView) convertView.findViewById(R.id.vp_tv_title_name1);
				// 终端
				TextView tv_termname = (TextView) convertView.findViewById(R.id.vp_tv_title_name3);
				// 产品名称
				TextView tv_produname = (TextView) convertView.findViewById(R.id.vp_tv_title_name0);
				// 分割线
				View vp_v_title_view = (View) convertView.findViewById(R.id.vp_v_title_view);
				// 添加
				Button btn_add = (Button) convertView.findViewById(R.id.vp_btn_add);
				// 操作
				TextView vp_tv_title_operation = (TextView) convertView.findViewById(R.id.vp_tv_title_operation);
				// 分割线
				View operation_divider = (View) convertView.findViewById(R.id.operation_divider);
				// ListView
				NoScrollListView mNoScrollListView = (NoScrollListView) convertView.findViewById(R.id.vp_lv);
				...
				// 本界面用到的变量
				// 空白终端
				final boolean isBlank = getResources().getString(R.string.plan_blantitle).equals(checkname);
				// 有效销售
				final boolean isPuHuo = getResources().getString(R.string.plan_puhuotitle).equals(checkname);
				// 有效铺货
				final boolean isXiaoShou = getResources().getString(R.string.plan_xiaoshoutitle).equals(checkname);
				// 道具生动化
				boolean isDaoJu = getResources().getString(R.string.plan_daojutitle).equals(checkname);
				// 产品生动化
				boolean isChanPin = getResources().getString(R.string.plan_product).equals(checkname);
				...
				if (isBlank || isPuHuo || isXiaoShou)// 空白终端跟checkname一样 当前是空白终端
				{
					...
					tv_produname.setVisibility(View.GONE);
					vp_v_title_view.setVisibility(View.GONE);
					if (preview_plan) {
				...
				makeplan_botton_ll.addView(convertView);
			}
			...
			makeplan_botton_ll.setVisibility(View.GONE);
	...
	private LinearLayout makeplan_botton_ll;// 底部计划模块容器
	...
	private void savePlan() {
		...
		MakePlanLinearLayout.this.finish();
	...
	public void ShowDiologWindow(final CopyOnWriteArrayList<MstProductM> productList, final String checkname) {
		...
		// 加载弹出窗口layout
		itemForm = MakePlanLinearLayout.this.getLayoutInflater().inflate(R.layout.operation_addrelation, null);
		productDialgo = new AlertDialog.Builder(
		// 设置对话框不能被取消
				MakePlanLinearLayout.this).setCancelable(false).create();
		// 设置视图显示在对话框中
				...
				if (colitemList.size() > 0) {
					...
					proLv.setOnItemClickListener(new OnItemClickListener() {
						...
						public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
							...
							caijixiangproductAdapter = new MakePlanProductAdapter(MakePlanLinearLayout.this, productList);
								...
								if (isDaoJu(checkname)) {// 添加道具 产品名称
									...
									for (MstProductM product : productes) {
										if (productnames.size() > 0 && productnames.contains(product.getProname())) {
											Toast.makeText(MakePlanLinearLayout.this, getString(R.string.addrelation_msg_repetitionadd), Toast.LENGTH_LONG).show();
										} else {
	...
	public void dialogProduct() {
		...
		// 加载弹出窗口layout
		itemForm = MakePlanLinearLayout.this.getLayoutInflater().inflate(R.layout.shopvisit_addrelation, null);
		productDialgo = new AlertDialog.Builder(MakePlanLinearLayout.this).setCancelable(false).create();
		productDialgo.setView(itemForm, 0, 0, 0, 0);
		productDialgo.show();
		...
		agencyAdapter = new ListViewKeyValueAdapter(MakePlanLinearLayout.this, ConstValues.agencyVieLst, new String[] { "key", "value" }, new int[] { R.drawable.bg_agency_up, R.drawable.bg_agency_down });
		...
		agencyLv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				productAdapter = new ChatVieProductAdapter(MakePlanLinearLayout.this, agency.getChildLst(), new String[] { "key", "value" }, new int[] { R.drawable.bg_product_up, R.drawable.bg_product_down });
		...
		if (!CheckUtil.IsEmpty(ConstValues.agencyVieLst)) {
			productAdapter = new ChatVieProductAdapter(MakePlanLinearLayout.this, ConstValues.agencyVieLst.get(0).getChildLst(), new String[] { "key", "value" }, new int[] { R.drawable.bg_product_up, R.drawable.bg_product_down });
			productLv.setAdapter(productAdapter);
		}
	...
	class PlanListViewAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				convertView = View.inflate(MakePlanLinearLayout.this, R.layout.operation_makeplan_listitem, null);
